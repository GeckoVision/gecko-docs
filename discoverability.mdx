...
description: surfcall exists to make an API discoverable to agents — question-shaped tools, intent-to-endpoint search, hidden auth, and an llms.txt for the API itself.
---

surfcall's whole reason to exist is **discoverability for agents**. A human can read
...
surfcall can emit an agent/human-readable capability map grouped by tag — the
...
Discoverability is real today on an ingested **OpenAPI 3.x** surface. surfcall does not
