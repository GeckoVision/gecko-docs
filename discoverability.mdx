...
<Note>
Shipped today: the MCP surface, `search_capabilities`, hidden auth, and a usable-only
surface. Rolling out (see [Roadmap](/roadmap)): the `x-gecko` OpenAPI and
`/.well-known/gecko.json` manifest, `search_capabilities` returning the full call recipe
inline (prerequisites + a worked example), and access-quality measurement — did the agent
find and correctly use the call. We build these against our own docs first.
</Note>
