...
- **The Program Surface** — comprehend Solana programs on-chain, recovering the PDA seeds an IDL drops; proven on four mainnet programs. See [Program Surface](/program-surface).
...
- **Multi-API correlations + the feedback path** — joining surfaces so an agent can chain calls across APIs, and capturing call outcomes (control-plane metadata only) to prove first-call-correctness as a spec drifts. An honest frontier, not a data moat.
...
- **Cloud** — hosted always-on watcher jobs, drift + poisoning alerts, and team entitlements. Cross-API correctness signal is an explored V2 direction, not a delivered capability.
...
always-on stay-correct watcher, drift + poisoning alerts, team entitlements, and the Agent-Readiness Scorecard + drift-watch. Who pays, in one sentence: **the engine is free forever for builders; API
