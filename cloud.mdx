...
<Note>
  **The engine is free and stays free.** Everything that makes an agent call an API
  ...
  hosted, continuously-correct, cross-customer layer. This page describes what it does.
</Note>
...
The free engine is the **funnel**: any developer points it at any API and gets
...
<CardGroup cols={2}>
  ...
  <Card title="Cloud — hosted, continuous, multiplayer (building)" icon="cloud-bolt">
    ...
    *Explored V2 direction (not delivered): a cross-API correctness signal drawn from
    across the fleet.*
  </Card>
</CardGroup>
...
<Note>
  Prices are **illustrative** — they land when Cloud ships. We're showing the shape now,
  not locking a number.
</Note>
...
the tools at your x402 endpoint (**PayAI, pay.sh, or a metering/billing rail**) as part of the setup. You keep
...
<Note>
  ...
  open source. The moat is execution: comprehension + comprehension-native security + auth
  handled day-one, combined — not a data corpus.
</Note>
