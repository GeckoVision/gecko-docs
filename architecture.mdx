---
title: Architecture
description: surfcall is a control plane, not a data plane. The invariants, the module map, and the single API-agnostic seam.
---
...
The agent calls the real API **directly** for data. surfcall injects credentials and
...
<CardGroup cols={2}>
  ...
  <Card title="Auth is invisible to the agent" icon="eye-slash">
    Tool defs never expose auth headers. The agent describes intent; surfcall injects
