...
<iframe
  src="/assets/architecture-context.html?embed=1"
  title="Gecko context diagram — interactive"
  style={{ width: "100%", height: "620px", border: "0", borderRadius: "12px" }}
></iframe>

<Note>Pan, zoom, switch themes, or use the guided views inside the diagram. [Open it full-screen](/assets/architecture-context.html).</Note>

---

## View 2 — The full pipeline (sources → knowledge → action)

<Frame caption="Untrusted sources pass an anti-poison gate into comprehension, become provenance-tagged graphs, project as MCP tools, and reach action only through simulate → receipt → signing gate.">
...
<Steps>
  ...
  <Step title="Action — verify, never execute">
    ...
    **external builder** builds → Gecko simulates → a **Receipt** → a fail-closed signing
    gate → an **external signer** (wallet / TEE / human).
  </Step>
  <Step title="Learn">
...
<iframe
  src="/assets/architecture.html?embed=1"
  title="Gecko pipeline diagram — interactive"
  style={{ width: "100%", height: "620px", border: "0", borderRadius: "12px" }}
></iframe>

<Note>Pan, zoom, switch themes, or use the guided views inside the diagram. [Open it full-screen](/assets/architecture.html).</Note>

---

## View 3 — The on-chain action path (the proven loop)

<Frame caption="Intent → derive the full account set → state reads → external build → unsigned prelude assembly → simulate on a $0 fork → Receipt → only a passing receipt reaches the signer.">
...
5. **Unsigned prelude assembly**, for simulation only — idempotent ATA creation, wSOL
   wrap/unwrap, compute budget.
6. **Simulate** — `simulateTransaction` with `sigVerify:false` on a $0 mainnet fork.
7. **Receipt** — status, categorical revert class, compute units. → [The Receipt](/receipt)
8. **Signing gate** — fail-closed. Only a passing verdict reaches an external signer.

Gecko never signs and never broadcasts. Unsigned assembly for simulation is the one
documented carve-out, and it is enforced at the sign/send boundary by an AST check that
proves the landing layer contains no signing or sending path.

<iframe
  src="/assets/architecture-onchain.html?embed=1"
  title="Gecko on-chain diagram — interactive"
  style={{ width: "100%", height: "620px", border: "0", borderRadius: "12px" }}
></iframe>

<Note>Pan, zoom, switch themes, or use the guided views inside the diagram. [Open it full-screen](/assets/architecture-onchain.html).</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="Never sign, never broadcast" icon="ban">
...
- Seven fail-closed security layers: spec sanitizer · per-tool quarantine · image Skill
  Guard · SSRF netguard · out-of-band auth anchoring · verdict signing gate · the
