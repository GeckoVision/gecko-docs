...
<AccordionGroup>
  ...
  <Accordion title="Still V2 (designed, not built)">
    A vectorized semantic index (today's catalog is lexical). **Drift-watch** — re-scoring
    a surface when its spec changes — is shipped; a fully-automatic
    re-comprehend-on-new-version job remains V2. See [Stay correct](/stay-correct).
  </Accordion>
</AccordionGroup>
