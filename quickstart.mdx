...
One line, any system — no Python prerequisite (downloads a prebuilt binary, or falls back to `uv`):

```bash
curl -fsSL https://get.geckovision.tech/install.sh | bash
```

Prefer a package manager? Run it straight from PyPI, or install the CLI + SDK:

```bash
uvx --from "gecko-surf[serve]" gecko <openapi-url>   # no install — run from PyPI
