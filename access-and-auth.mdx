---
title: Access & auth
description: The whole engine/adapter seam is one function — Session.auth_headers(). How surfcall handles auth and paywalls invisibly to the agent.
---

Auth is invisible to the agent. The agent describes intent; surfcall injects
...
When a session returns an **empty** auth-header dict, surfcall treats it as "this
...
has to learn. surfcall encodes it so the agent only ever describes intent:
...
it. surfcall does not sign or broadcast mainnet transactions on its own.
</Warning>

## What surfcall never does with credentials
