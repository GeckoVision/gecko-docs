...
The same comprehension runs on-chain: [The Program Surface](/program-surface) recovers the
PDA seeds a Solana IDL drops and derives the accounts first-plan-correct.

![Gecko turns a raw OpenAPI operation into a question-shaped tool with a JSON schema — auth header removed](/images/rawtool.gif)
...
This is **lexical, not vector** search, and that is a design choice rather than a
milestone: the graph never *approximately* remembers. BM25 and semantic tiers both sit
behind evidence gates that flip only on **measured** lexical recall failure — fed by a
misrank-aware evaluation (a golden set plus a closed miss-cause vocabulary). One measured
negative result on embeddings is already on record. See [find_start](/find-start) for the
on-chain twin of this router and the honesty floor it enforces.
