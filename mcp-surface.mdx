...
A Solana [Program Surface](/program-surface) is served the same way, via
`gecko-orquestra --program <name>`.

<Tip>
Want to see it without installing anything? The flagship hosted surface is one line in
Claude Code:
```bash
claude mcp add --transport http gecko https://mcp.geckovision.tech/orquestra/mcp
```
That's 16 tools over Solana storefronts, Orca Whirlpool swaps, `find_start` and program
graphs. Every other client's wiring is in
[`mcp-config.json`](https://www.geckovision.tech/mcp-config.json), and the runbook is
[`agents.md`](https://www.geckovision.tech/agents.md). For a hosted HTTP API instead:
```bash
...
In Claude Code you can instead `/plugin install gecko-surf@geckovision`, which wires that
