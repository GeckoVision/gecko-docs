...
surfcall exposes an ingested API to an agent through the **Model Context Protocol
...
<Steps>
  ...
  <Step title="Call">
    Call the tool by name with the inputs. surfcall builds the correct request,
...
```python
from surfcall.mcp_server import serve_stdio
